	...
	export default {
		...
		computed: {
			...
			rows() {
				...
				const dateCountOfLastMonth = getDayCountOfMonth(date.getFullYear(), (date.getMonth() === 0 ? 11 : date
					.getMonth() - 1));
				...
				for (let i = 0; i < 6; i++) {
					...
					for (let j = 0; j < 7; j++) {
						...
						cell.disabled = cellDate < new Date() - 8.64e7;
		...
		methods: {
			...
			getCellClasses(cell) {
				...
				const defaultValue = this.defaultValue ? Array.isArray(this.defaultValue) ? this.defaultValue : [this
					.defaultValue
				] : [];
				...
				if (selectionMode === 'day' && (cell.type === 'normal' || cell.type === 'today') && this.cellMatchesDate(
						cell, this.value)) {
