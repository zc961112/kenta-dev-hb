...
const service = axios.create({
  ...
  baseURL: appConfig.VUE_APP_BASE_API,
  // 超时
  timeout: 100000
})
...
service.interceptors.request.use(config => {
  ...
  if (config.headers && config.headers.formData) {
    isFormData = true;
...
service.interceptors.response.use(res => {
  ...
  if (code === 401) {
    if (!isRelogin.show) {
      isRelogin.show = true;
      MessageBox.confirm('The login status has expired. You can continue to stay on this page or log in again', 'Tips', {
    ...
    return Promise.reject('Login has expired, please log in again。')
  } else if (code === 500) {
    // Message({
    //   message: msg,
    //   type: 'error'
