import { getDestinationList, destinationSlugDetail } from '@/api/destination'
import { getHotelRooms } from '@/api/hotel'
import { mapToken } from '@/config/map'
import { updateSessionQuery, getHotelRoomsParams } from '@/utils/casino'
...
export default {
  ...
  methods: {
    ...
    async handleSearch () {
      this.markerList = []
      try {
        const params = { slug: this.searchQuery.destinationId }
    ...
    async handleEachPrice (p, index) {
      try {
        ...
        if (hotel) {
          ...
          this.$set(casino, '$tax', (paymentInfo.tax_data.taxes || []).reduce((total, item) => Decimal.add(item.amount, total), 0))
    ...
    async initMapMarkers () {
      try {
        ...
        if (mapRes) {
          this.form.casinoList.forEach((item, index) => {
            if (this.map) {
              if (index === 0) {
                this.map.flyTo({ center: [item.longitude, item.latitude] })
              }

              const popup = new mapboxgl.Popup({ offset: 25 })
                .setHTML(`<div class="map-popper">
                  <img src="${item.imagesList[0].url}" class="map-pp-img">
                  <h3 class="map-pp-title">${item.name}</h3>
                  <div class="map-pp-text">${item.cityName}, ${item.countryName}</div>
                  <div class="map-pp-line"></div>
                  <div class="map-pp-price">From <span class="map-pp-price-no">€${(item.$price || 0).toFixed(2)}</span> night</div>
                </div>`)

              const el = document.createElement('div')
              el.id = 'marker_' + item.slug
    ...
    handleRoomSelect ({ info, item, hotel }) {
    ...
    handleQuerySelect (item) {
      this.$router.push({
        ...
        query: Object.assign(this.$route.query, { destinationName: item.value })
      })
